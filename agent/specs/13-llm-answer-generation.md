...
When present, at most three session-memory summaries and 2,000 characters are placed in a separate `<memory_context>` block. They may resolve references in the current question but are untrusted, non-authoritative and cannot support a `[Cn]` claim. Memory-only answers bypass this KB generator and use the workflow's `memory_grounded` terminal contract.
